...
<div class="container">	
	...
	<br><br>
		<div>
	<form:form action="ajouter" method="POST" class="form-horizontal" commandName="film">
...
<%-- 				<c:forEach items="${film.acteurs}" var="acteur" varStatus="status"> --%>
				<c:forEach begin="0" end="4" step="1"  varStatus="status">
					<div class="form-group">			
						<label for="duree" class="col-xs-2 control-label">Acteur ${status.current + 1} :</label>
						<div class="col-xs-5">
							<input name="acteurs[${status.current}].prenom" placeholder="Prénom ..." class="form-control"/>
						</div>
						<div class="col-xs-5">
							<input name="acteurs[${status.current}].nom" placeholder="Nom ..." class="form-control"/>
						</div>
					</div>
				</c:forEach>
